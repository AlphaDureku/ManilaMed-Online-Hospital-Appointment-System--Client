...
export default function BookingConfirmation(props) {
  ...
  function getBookingDetails() {
    const { recom_Time, schedule_date, doctor_ID } = props.appointmentDetails;
  ...
  function getdoctorFullName() {
    const bookingdetails = props.doctors
      ? props.doctors.filter(
          (details) =>
            details.doctor_ID ===
            getBookingDetails().map((details) => details.doctor_ID).join(", ")
        )
      : [];
    ...
    <div>
      <Container>
        ...
        <Row className="mt-3">
          ...
          {getBookingDetails() ? (
             getBookingDetails().map((details) => getFormattedDate(details.schedule_date)).join(", ")) : ( "N/A")}

          </div>
          <div className="reqscheddate mb-3">
            {getBookingDetails() ? ( getBookingDetails().map((details) => details.recom_Time)
              .join(", ")): ("N/A")}
          </div>
        ...
        <Row className="mb-5">
          ...
          {getdoctorFullName() ? ( getdoctorFullName().map((details) => details.doctorFullName)
              .join(", ")): ("N/A")}
          </div>
          <div className="reqdoctorspec mb-3">
          {getdoctorFullName() ? ( getdoctorFullName().map((details) => details.doctorSpecialization)
              .join(", ")): ("N/A")}
          </div>
