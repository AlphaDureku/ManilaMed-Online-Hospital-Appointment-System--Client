...
export default function Card(props) {
  ...
  function getAssignedSched(doctorID) {
    const schedules = props.schedule.filter((sched) => doctorID === sched.doctor_ID);
...
//To display the time of the selected date
  function renderSchedule() {
    ...
    const selectedDateSchedules = selectedDoctorSchedules.filter((sched) => {
      return selectedDate && selectedDate.toDateString() === new Date(sched.day).toDateString();
    });
  ...
  function getDayProps(date) {
    const isScheduled = props.schedule.some((sched) => {
      return date.toDateString() === new Date(sched.day).toDateString() && sched.doctor_ID === selectedDoctorId;
    });
